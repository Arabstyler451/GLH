(function () {
    ...
    /* On page load, mark the pre-filled postcode as already valid visually */
    if (postcodeInput && postcodeInput.value.trim()) {
        postcodeInput.classList.add('input-valid');
    }
    ...
    if (postcodeInput) {
        ...
        postcodeInput.addEventListener('blur', async function () {
            ...
            try {
                ...
                /* API unreachable — fail silently, don't block save */
                setPostcodeStatus('', '');
    ...
    if (streetInput) {
        if (streetInput.value.trim()) streetInput.classList.add('input-valid');
    ...
    if (cityInput) {
        if (cityInput.value.trim()) cityInput.classList.add('input-valid');
